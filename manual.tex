...
\lstdefinelanguage{YAML}{
  keywords={states, initial, transitions, guard, action, to, do, entry, exit, orthogonal, hooks, decisions, context, context_init, includes, history, lang},
...
\section{Overview}
This tool generates a safety-critical Hierarchical State Machine (HSM) from a YAML/SMB definition file. It supports two output languages --- \textbf{Rust} and \textbf{C} --- and includes advanced features such as orthogonal regions (parallelism), history patterns, decision trees, and fork transitions.
...
\texttt{-{}-lang} & from file & Output language: \texttt{rust} or \texttt{c}. If omitted, reads the \texttt{lang:} key from the SMB file (defaults to \texttt{rust} if absent). The \texttt{lang:} key may be a list, in which case code is generated for all listed languages. \\
\texttt{-o}, \texttt{-{}-output} & \texttt{./statemachine} & Output base path without extension. Language-specific extensions (\texttt{.rs}, \texttt{.c}, \texttt{.h}, \texttt{.dot}) are appended automatically. Parent directories are created if needed. \\
\bottomrule
\end{longtable}
...
Both & \texttt{name.dot} & Graphviz visualization of the state machine. \\
\bottomrule
\end{longtable}
...
\begin{longtable}{@{}lll@{}}
\toprule
\textbf{Element} & \textbf{Rust} & \textbf{C} \\
\midrule
\endhead
Context access & \texttt{ctx.field} & \texttt{ctx->field} \\
Context fields & \texttt{pub field: Type,} & \texttt{Type field;} \\
Context init & \texttt{field: value,} & \texttt{sm->ctx.field = value;} \\
Print & \texttt{println!("...")} & \texttt{printf("...\textbackslash n")} \\
String type & \texttt{\&str} & \texttt{const char*} \\
Boolean & \texttt{bool} & \texttt{bool} (via \texttt{stdbool.h}) \\
\bottomrule
\end{longtable}
...
Both backends generate a \texttt{Context} struct (Rust: \texttt{Context}, C: \texttt{SM\_Context}) containing:
...
\midrule
\endhead
\texttt{ctx} & All & The shared context struct. Rust: \texttt{\&mut Context}. C: \texttt{SM\_Context*}. \\
...
Setting \texttt{to: null} causes the machine to exit all states from the current state up through root, then stop. After termination, \texttt{is\_running()} (Rust) or \texttt{sm\_is\_running()} (C) returns false.
...
\textbf{Note:} Use the \texttt{IN\_STATE(X)} macro in guards for portable state checks. In Rust, it expands to \texttt{ctx.in\_state\_X()}. In C, it expands to the \texttt{IN\_STATE\_X} macro.
